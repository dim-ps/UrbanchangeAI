...
Pipeline per image:
  ...
  7. (optional, --use-vlm) A local VLM (Qwen2.5-VL) re-inspects each building's
     ground-floor crop (with CLIP's verdict + OCR text as context), re-classifies
     its use with a short reasoning phrase, and fuses its verdict with CLIP's.
...
# 0.35 was measured (via a raw-score dump across flagged panoramas) to systematically
# drop real, unoccluded buildings that score 0.28-0.34 -- lowering to 0.25 recovered every
# missing building in the audited images with zero new false positives (stable results
# down to 0.22, i.e. the extra headroom below 0.25 wasn't hiding more real candidates).
BOX_THRESHOLD = 0.25
TEXT_THRESHOLD = 0.25
GF_FRACTION = 0.30                    # bottom-fraction fallback when no shopfront features are found
GF_FALLBACK_MAX_FRAC = 0.12           # ... but never taller than this fraction of the whole image height:
                                      # gf_fraction*building_height grows with the building's height, so a
                                      # tall multi-storey facade got a fallback band bleeding into the 1st
                                      # floor; ground floors don't get taller just because a building has
                                      # more storeys, so cap the fallback band in absolute (image-relative) terms
...
FEATURE_MAX_HEIGHT_FRAC = 0.35        # a real shopfront element (door/window/sign/awning) never spans more
                                      # than this fraction of the image height; GDINO occasionally returns a
                                      # box for "storefront" that loosely bounds an entire multi-storey facade
                                      # instead of one element -- letting that into feat_mask made the feature-
                                      # based scan walk almost to the roofline (measured gf_height_frac up to
                                      # 1.00 on real buildings), so oversized feature boxes are dropped before
                                      # they're rasterized
...
MASK_DEDUP_OVERLAP = 0.5             # two detections whose SAM masks overlap more than this (as a fraction
                                     # of the smaller mask) are the same physical building -- drop the later
                                     # one. detect()'s NMS runs on raw GDINO boxes and TILE_SCALE_FRACS can
                                     # produce two boxes for one facade whose boxes don't overlap enough to
                                     # trip box-level NMS but whose post-SAM masks clearly do
...
# A second, narrower tile scale in addition to the full-height square tile: a facade that
# appears "thin" relative to a full-height window (compressed by the equirectangular
# projection, or simply narrow) can score too low to pass BOX_THRESHOLD there, while a
# tighter crop that fills more of the frame with just that facade pushes its score up --
# measured directly on a building that only cleared threshold once cropped to a narrower
# window. Fractions are of the full-height tile width.
TILE_SCALE_FRACS = (1.0, 0.6)
...
# -- Step 7: VLM ground-floor re-classification --------------------------------
# CLIP gives each building's ground-floor crop ONE zero-shot use-class, which is
# brittle on cluttered storefronts. A small local VLM re-inspects the same crop
# (with the CLIP verdict + OCR text as context) and returns a re-reasoned
# verdict -- use-class + confidence + a short evidence phrase explaining what it
# saw or read. No spatial unit-splitting: one crop, one call, one verdict.
#
...
VLM_CLASSIFY_PROMPT_TEMPLATE = """You are looking at a building's ground-floor crop from a street-level photo.

OCR text detected on this ground floor (may be empty, partial, garbled, or contain unrelated street clutter
like watermarks or plate numbers): "{ocr_text}"

If a word or phrase in that OCR text clearly names a business, product, or service and looks like it belongs
to what you see (e.g. "nail salon", "pharmacy", "bakery", "ATM", "hotel", "garage"), use it as
...
Classify this ground floor into EXACTLY ONE of these categories: {classes}
...
Guidance for mapping what you see/read to these categories:
  ...
  - gym, wellness -> "Health and wellness establishments"
  - nail salon, cosmetics, clothing, jewelry -> "Beauty and fashion boutiques"
  - bank, ATM, office, real estate, insurance, legal, pharmacy, clinic, hairdresser -> "Professional services"
  - open-air parking, garage door, car park -> "Parking lot"
  - hotel, short-term rooms -> "Accommodation"
...
class GroundFloorSegmenter:
    ...
    def detect(self, image_rgb, prompt, box_threshold, text_threshold, tile=True):
        ...
        if tile and w / h > TILE_ASPECT_TRIGGER:
            for frac in TILE_SCALE_FRACS:                # full-height tile + a narrower scale
                tile_w = max(1, int(h * frac))
    ...
    def _ground_floor_region(self, mask, feat_mask, single_storey, gf_fraction):
        ...
        # Fallback: no features on this facade. gf_fraction*bh grows with the building's own
        # height, so a tall multi-storey facade got a band bleeding well into the 1st floor --
        # a real ground floor doesn't get taller just because the building has more storeys, so
        # cap the band in image-relative (not building-relative) terms too.
        band_h = min(gf_fraction * bh, GF_FALLBACK_MAX_FRAC * mask.shape[0])
        band_top = int(round(y_bot - band_h))
        gf[:band_top] = False
    ...
    def segment(self, image_rgb, boxes, gf_fraction, feature_boxes=None):
        ...
        for fb, _ in (feature_boxes or []):
            fx0, fy0, fx1, fy1 = fb
            if fy1 - fy0 > FEATURE_MAX_HEIGHT_FRAC * h:      # not a real door/window/sign -- GDINO
                continue                                     # occasionally matches "storefront" to a box
                                                              # spanning most of the facade instead of one
                                                              # element, which drags the ground-floor scan
                                                              # almost to the roofline (see FEATURE_MAX_HEIGHT_FRAC)
        ...
        for box, score in boxes:
            ...
            # mask-level dedup: boxes already passed box-level NMS in detect(), but the two
            # tile scales can still produce two boxes for the same facade whose SAM masks
            # overlap heavily even though the raw boxes didn't overlap enough to trip NMS.
            # `boxes` is score-sorted, so the first (highest-score) mask for a building wins.
            if any((building_mask & r["mask"]).sum() > MASK_DEDUP_OVERLAP * min(area, r["mask"].sum())
                   for r in results):
                continue
    ...
    def vlm_correct_building(self, image_rgb, building, full_res_image=None, scale=1.0):
        """Runs step 7 on one building: a single VLM call re-classifies the
        whole ground-floor crop (CLIP's verdict + OCR text as context) and
        returns a reasoned verdict. Mutates and returns `building` with:
          - vlm_evidence: the VLM's short reasoning phrase, or "" if the VLM
            is disabled or the call failed/was unparseable.
          - use_class / confidence / bucket: overwritten with the VLM's verdict
            if it produced a confident, valid result; otherwise left as CLIP's
            original verdict (nothing is lost).
        ...
        crop = fit_crop(src[fy0:fy1, fx0:fx1])
        if crop.size == 0:
            return building
        ...
        cls_raw = self._vlm_generate(crop, classify_prompt, VLM_CLASSIFY_MAX_NEW_TOKENS)
        result = self._parse_vlm_classification(cls_raw)
        if result is None:
            return building                              # VLM disabled/failed/unparseable -> keep CLIP verdict
        building["vlm_evidence"] = result["evidence"]

        # Take the VLM label only when it agrees with CLIP, or disagrees with
        # reasonably high confidence, so a low-confidence VLM call never
        # overrides a strong CLIP verdict.
        agree = result["use_class"] == building.get("use_class")
        if not agree and result["confidence"] < 0.55:
        ...
        building["use_class"] = result["use_class"]
        building["confidence"] = result["confidence"]
        building["bucket"] = ("high" if result["confidence"] >= 0.7
                              else "medium" if result["confidence"] >= 0.45 else "low")
...
def render(image_rgb, buildings, gf_fraction):
    ...
    for k, i in enumerate(order):
        ...
        gf_label = _truncate_text("Ground Floor Use:", f_small, box_w - 2 * pad, draw)
        ...
        if b.get("vlm_corrected"):
            ...
            ty += 20
        vlm_evidence = b.get("vlm_evidence", "")
        if vlm_evidence:
            evid_line = _truncate_text(f'VLM: "{vlm_evidence}"', f_small, box_w - 2 * pad, draw)
            draw.text((x0 + pad, ty), evid_line, font=f_small, fill=(160, 220, 255, 255))
            ty += 20
    ...
    if n_vlm_corrected:
        lines.append(f"VLM: {n_vlm_corrected} reclassified")
...
def process_image(seg, image_path, out_path, prompt, box_threshold, text_threshold,
    ...
    # sidecar JSON with the VLM verdict + reasoning (kept separate from the
    # rendered PNG so the raw structured data survives even when the render
    # truncates the evidence text per building).
    if use_vlm:
        sidecar = [{
            "building": i + 1,
    ...
    n_arcade = sum(1 for b in buildings if b.get("single_storey"))
    n_vlm_corrected = sum(1 for b in buildings if b.get("vlm_corrected"))
    extra = f", VLM: {n_vlm_corrected} reclassified" if use_vlm else ""
...
def main():
    ...
    ap.add_argument("--use-vlm", action="store_true",
                    help="enable step 7: VLM ground-floor re-classification with reasoning")
